...
@section('content')
	...
	<section class="content-header">
	  ...
	  <ol class="breadcrumb">
	    <li><i class="fa fa-book"></i> Daftar Harga</li>
	  </ol>
	</section>
		...
		<div class="row">
			<div class="col-md-12">
				<div id="radioBtn" class="btn-group">
					<a class="btn btn-tumblr btn-sm notActive" data-toggle="fun" data-title="Y" onclick="showSatu()" style="margin-right: 3px;">By Kode </a>
					<a class="btn btn-tumblr btn-sm notActive" id="btnNama" data-toggle="fun" data-title="N" onclick="showDua()" style="margin-right: 3px;">By Nama</a>
					<a class="btn btn-tumblr btn-sm notActive" data-toggle="fun" data-title="Z" onclick="showTiga()" >By Suplier</a>
				</div>
			</div>
		</div>
		<br>

		@include('item.partials.formitem')
		@include('item.partials.resultitem')
		@include('item.partials.modal')
  ...
  $(document).ready(function () {
	...
	<script>
		function showSatu(){
			...
			$("#pilih").select2("val",{placeholder: 'ALL Suplier'}); //set the value
			$("#pilihprodusen").select2("val",{placeholder: 'Pilih Suplier'}); //set the value
					...
					success: function(data) {
						...
						var table = $('#databarang').DataTable({
							...
							buttons: [

								{
					                ...
					                init: function(api, node, config) {
					                  $(node).removeClass('dt-button ui-button ui-state-default')
					                },
					                  ...
					                  // console.log(data.footer[6]);
					                    for (var i=0; i<data.body.length; i++){
					                        for (var j=0; j<data.body[i].length; j++ ){
					                            ...
					                            data.body[i][2] = data.body[i][2].replace(/\./g,""); //dpp
					                            data.body[i][3] = data.body[i][3].replace(/\./g,""); //dpp
					                            data.body[i][4] = data.body[i][4].replace(/\./g,""); //dpp
					                            // data.body[i][5] = data.body[i][5].replace(/\./g,""); //ppn
					                        }
					                    }
							...
							columns: [
								...
								{ data: 'harga_jawa', width: '20px', "render": $.fn.dataTable.render.number( '.', ',', 0 ),"sClass": "text-right" },
								{ data: 'harga_luar_jawa', width: '20px', "render": $.fn.dataTable.render.number( '.', ',', 0 ),"sClass": "text-right" },
								{ data: 'harga_batam', width: '10px', "render": $.fn.dataTable.render.number( '.', ',', 0 ),"sClass": "text-right" },
								{ data: 'suplier', width: '400px' },
								{
									"data": null,
						...
						// $('#databarang tbody').on( 'click', '.viewdetail', function () {
						//     var data = table.row( $(this).parents('tr') ).data();
						//     alert( data.id +"'s salary is: "+ data.nama );
						// } );
						...
						$('#databarang tbody').on( 'click', '.viewdetail', function () {
								...
								document.getElementById('nabar').value =data.nama;
								var idbarang =data.id;
								$.ajax({
									type: 'POST',
									url: 'item/'+idbarang,
									data: {"_token": "{{ csrf_token() }}","id": idbarang,"_method":"post"},
									cache:false,
									success: function(detail) {
										// console.log(detail);
										$('#tglstok').html(detail.tanggal);
										document.getElementById('stokbar').value =detail.stok;
									}
								});
						});
					}
