...
class HomeController extends Controller
{
  ...
  public function selectcust_erp(Request $request)
  {
    $cabang = Auth::user()->kode_cabang;
    $custs = DB::connection('mysql_erp')->table('er_md_member')->select('id_member', 'nama_member', 'no_cust', 'alamat_detail')
      ->whereRaw('SUBSTRING_INDEX(no_cust, ".", 1) =  "' . $cabang . '"')
      ->where('nama_member', 'like', "%{$request->search}%")->get();
    if (!empty($custs[0]->id_member)) {
      foreach ($custs as $cust) {
        $custArray[] = array(
          "id" => $cust->id_member,
          "text" => $cust->nama_member . " - " . $cust->alamat_detail,
        );
      }
    } else {
      $custArray[] = array(
  ...
  public function selectcustnama(Request $request)
  {
    ...
    if (Auth::user()->kode_cabang === 'CDI') {
      $custs = DB::connection('mysql_erp')->table('er_md_member')->select('id_member', 'no_cust', 'nama_member');
    } elseif (Auth::user()->kode_cabang !== 'CDI') {
      $custs = DB::connection('mysql_erp')->table('er_md_member')->select('id_member', 'no_cust', 'nama_member')
        ->whereRaw('(SUBSTRING_INDEX(no_cust, ".", 1) ="' . $cabang2 . '" OR SUBSTRING_INDEX(no_cust, ".", 1) = "000")');
    }

    $custs2 = $custs->where('nama_member', 'like', "%{$request->search}%")->get();
    if (!empty($custs2[0]->id_member)) {
      foreach ($custs2 as $cust2) {
        $custArray2[] = array(
          "id" => $cust2->no_cust . " - " . $cust2->nama_member,
          "text" => $cust2->no_cust . " - " . $cust2->nama_member,
        );
      }
    } else {
      $custArray2[] = array(
