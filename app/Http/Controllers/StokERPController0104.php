...
class StokERPController extends Controller
{
    ...
    public function data_stok_erp(Request $request)
    {
        ...
        if (Auth::user()->kode_cabang === 'CDI') {
            $log = DB::connection('mysql_erp')->select('SELECT id_stock, er_md_product.code, er_md_product.deskripsi, nama_perusahaan, SUM(qty) AS qty, harga_per_satuan_jual, (harga_per_satuan_jual*1.1) AS hrgppn, (harga_per_satuan_jual_non_jawa*1.1) AS hrgluarjawappn, price_category,
            IF(price_category="jawa",(harga_per_satuan_jual*1.1),(harga_per_satuan_jual_non_jawa*1.1)) AS fix_harga FROM er_wr_product_stock 
    ...
    public function stock_peritems(Request $request)
    {
        ...
        $items =  collect(DB::connection('mysql_erp')->select('SELECT id_product, er_md_product.code as id, er_md_product.deskripsi as nama, nie, harga_per_satuan_jual, (harga_per_satuan_jual*1.1) AS hrgppn, (harga_per_satuan_jual_non_jawa*1.1) AS hrgluarjawappn, nama_perusahaan as produsen  FROM er_md_product 
    ...
    public function data_list_harga(Request $request)
    {
        ...
        if ($request->filled('kdbarang')) {
            ...
            (harga_per_satuan_jual*1.1) AS hrgppn, harga_per_satuan_jual, (harga_per_satuan_jual_non_jawa*1.1) AS hrgluarjawappn 
            FROM er_md_product 
