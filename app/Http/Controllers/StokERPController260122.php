...
class StokERPController extends Controller
{
    ...
    public function data_stok_erp(Request $request)
    {
        ...
        if (Auth::user()->kode_cabang === 'CDI') {
            $log = DB::connection('mysql_erp')->select('SELECT id_stock, er_md_product.code, er_md_product.deskripsi, nama_perusahaan, qty, harga_per_satuan_jual, (harga_per_satuan_jual*1.1) AS hrgppn FROM er_wr_product_stock 
            ...
            $log = DB::connection('mysql_erp')->select('SELECT id_stock, er_md_product.code, er_md_product.deskripsi, nama_perusahaan, SUM(qty) AS qty, harga_per_satuan_jual, (harga_per_satuan_jual*1.1) AS hrgppn FROM er_wr_product_stock 
            JOIN er_md_struktur_organisasi ON er_wr_product_stock.id_organisasi = er_md_struktur_organisasi.id_organisasi
            JOIN er_md_product ON er_wr_product_stock.id_product = er_md_product.id_product
            WHERE kode_cabang = "' . Auth::user()->kode_cabang . '" GROUP BY code');
        }
    ...
    public function stock_peritems(Request $request)
    {
        ...
        $items =  collect(DB::connection('mysql_erp')->select('SELECT id_product, er_md_product.code as id, er_md_product.deskripsi as nama, nie, harga_per_satuan_jual, (harga_per_satuan_jual*1.1) AS hrgppn, nama_perusahaan as produsen  FROM er_md_product 
    ...
    public function data_list_harga(Request $request)
    {
        ...
        if ($request->filled('kdbarang')) {
            $data = DB::connection('mysql_erp')->select('SELECT er_md_product.code, er_md_struktur_organisasi.kode_cabang AS kode, er_md_product.deskripsi, 
            ...
            (harga_per_satuan_jual*1.1) AS hrgppn, harga_per_satuan_jual 
            FROM er_md_product 
    ...
    public function viewdetail(Request $request)
    {
        ...
        $data =  collect(DB::connection('mysql_erp')->select('SELECT id_stock, er_md_product.code, er_md_product.deskripsi, nama_perusahaan, SUM(qty) AS stok
        FROM er_md_product
                    LEFT JOIN er_md_struktur_organisasi ON er_md_product.id_organisasi = er_md_struktur_organisasi.id_organisasi
                    LEFT JOIN er_wr_product_stock ON er_wr_product_stock.id_product = er_md_product.id_product
                    WHERE er_md_product.id_product = "' . $kobar . '" AND kode_cabang = "' . $cabang . '" GROUP BY CODE'))->first();
